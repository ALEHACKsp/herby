...
bool WorldToScreen( const Vector& world, Vector2D& screen )
{
	...
	auto& viewport = source::m_viewport;

	auto x = static_cast< float >( viewport.Width ) / 2.f;
	auto y = static_cast< float >( viewport.Height ) / 2.f;

	x += 0.5f * screen[ 0 ] * static_cast< float >( viewport.Width ) + 0.5f;
	y -= 0.5f * screen[ 1 ] * static_cast< float >( viewport.Height ) + 0.5f;
